...
export function AdminSellerRecentOrders({ recentOrders }: AdminSellerRecentOrdersProps) {
  return (
                          <Card className="bg-white/[0.02] border border-white/10 rounded-card overflow-hidden shadow-2xl">
                            <div className="p-6 md:p-8 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                              <div className="flex items-center gap-3">
                                ...
                                <h4 className="text-sm font-semibold text-white uppercase tracking-widest">Operational Stream</h4>
                              </div>
                            ...
                            <div className="overflow-x-auto">
                              <table className="w-full text-left">
                                <thead className="bg-white/[0.03] text-[10px] font-semibold text-gray-500 uppercase tracking-widest border-b border-white/5">
                                  <tr>
                                ...
                                <tbody className="divide-y divide-white/5">
                                    ...
                                    <tr key={String(order.id)} className="text-sm hover:bg-white/[0.02] transition-colors group/row">
                                      ...
                                      <td className="px-8 py-6">
                                        <div className="flex items-center gap-3">
                                          ...
                                          <span className="text-white font-semibold tracking-tight">{order.buyerName}</span>
                                        </div>
                                      </td>
                                      <td className="px-8 py-6">
                                        <span className="text-white font-semibold italic text-lg tracking-tight">KSh {order.totalAmount?.toLocaleString()}</span>
                                      </td>
                                      <td className="px-8 py-6">
                                        <Badge className={`
                                  ...
                                  {(!recentOrders || recentOrders.length === 0) && (
                                    <tr>
                                      <td colSpan={4} className="px-8 py-12 text-center text-gray-500 font-semibold uppercase tracking-widest text-xs opacity-50">
