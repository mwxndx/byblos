  ...
  return (
    <Card className="bg-surface-1 backdrop-blur-2xl border border-separator rounded-card overflow-hidden shadow-2xl">
      <CardHeader className="p-5 md:p-8 border-b border-separator bg-white/[0.01] flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
        <div>
          <CardTitle className="text-2xl md:text-3xl font-semibold text-label tracking-tight">Clients</CardTitle>
          <CardDescription className="text-xs md:text-sm text-label-2 font-medium">Customers and the sellers they buy from</CardDescription>
        </div>
        <div className="relative group w-full md:w-auto">
          ...
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-label-3 group-hover:text-pink-500 transition-colors" />
          <Input
            type="text"
            aria-label="Search clients"
            placeholder="Search clients..."
            className="pl-12 w-full md:w-[320px] lg:w-[400px] h-11 md:h-12 bg-fill border-separator text-label placeholder:text-label-3 rounded-2xl focus:border-pink-500/50 focus:ring-pink-500/10 transition-all font-medium text-sm"
            value={searchQuery}
                ...
                <tr key={String(client.id)} className="hover:bg-white/[0.02] transition-all group">
                  <td className="px-5 md:px-8 py-4 md:py-6">
                    <div className="flex items-center gap-3 md:gap-5">
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-fill flex items-center justify-center border border-separator group-hover:border-pink-500/30 transition-all shadow-inner">
                        <Users2 className="w-4 h-4 md:w-5 md:h-5 text-label-3 group-hover:text-pink-500 transition-all" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm md:text-base font-semibold text-label tracking-tight truncate">{String(client.name || '')}</p>
                        <p className="text-[9px] md:text-[10px] font-semibold text-label-3 uppercase tracking-widest opacity-50 truncate">CID: {String(client.id).slice(0, 12)}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 md:px-8 py-4 md:py-6 hidden md:table-cell">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-label-2 tracking-tight">{String(client.sellerName || '')}</span>
                    </div>
                  </td>
                  <td className="px-5 md:px-8 py-4 md:py-6 hidden lg:table-cell">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-label-2 italic lowercase">{String(client.email || '')}</span>
                    </div>
                  </td>
                  <td className="px-5 md:px-8 py-4 md:py-6 text-right">
                    <p className="text-[10px] md:text-sm font-bold text-label-2 tabular-nums">{formatDate(client.createdAt as string)}</p>
